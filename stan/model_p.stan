...
model {
    ...
    for (i in 1:N) {
        target += ratcliff_lpdf( y[i] | alpha[participant[i]], ter[participant[i]], .5, delta[participant[i],cond_coher[i]], eta[participant[i]]);
    }
}
generated quantities {
    vector[N] log_lik;    
    for (i in 1:N) {
        log_lik[i] = ratcliff_lpdf(y[i] | alpha[participant[i]], ter[participant[i]], .5, delta[participant[i],cond_coher[i]], eta[participant[i]]);
    }
}