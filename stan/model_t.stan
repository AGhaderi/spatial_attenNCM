...
parameters {
    ...
    real<lower=0> tersd;   // Between-condition-participant variability in non-decision time 
    ...
    vector<lower=0>[nspats] terhier;      // Hierarchical Non-decision time and spatial attention conditions
    ...
    matrix<lower=0, upper=1>[nparts,nspats] ter;   // Non-decision time for each participant and spatial spatial
...
model {
    ...
    for (p in 1:nparts) {
        ...
        for (c2 in 1:nspats) {
            ter[p,c2] ~ normal(terhier[c2], tersd) T[0, 1];
        }
    ...
    for (i in 1:N) {
        target += ratcliff_lpdf( y[i] | alpha[participant[i]], ter[participant[i],cond_spat[i]], .5, delta[participant[i],cond_coher[i]], eta[participant[i]]);
    }
}
generated quantities {
    vector[N] log_lik;    
    for (i in 1:N) {
        log_lik[i] = ratcliff_lpdf(y[i] | alpha[participant[i]], ter[participant[i],cond_spat[i]], .5, delta[participant[i],cond_coher[i]], eta[participant[i]]);
    }
}