...
# Code for calculate_waic was taken from bellow
...
def plot_sim_data(simdata,
    ...
    density1 = gaussian_kde(x1, bw_method='scott')
    x1d = np.linspace(min_x1, max_x1, gridsize)
    y1d = density1(x1d)

    density2 = gaussian_kde(x2, bw_method='scott')
    x2d = np.linspace(min_x2, max_x2, gridsize)
    y2d = density2(x2d)

    plt.plot(x1d, y1d, color=color[1])
    plt.plot(x2d, y2d, color=color[2])

    if show_intervals is not None:
        if np.sum(show_intervals == np.array(['BCI', 'HDI'])) < 1:
            raise ValueError("must be either None, BCI, or HDI")
        if show_intervals == 'BCI':
            low1, high1 = bci(x1, alpha_intervals)
            low2, high2 = bci(x2, alpha_intervals)
        else:
            low1, high1 = hdi(x1, alpha_intervals)
            low2, high2 = hdi(x2, alpha_intervals)
        plt.fill_between(x1d[np.logical_and(x1d >= low1, x1d <= high1)],
                        y1d[np.logical_and(x1d >= low1, x1d <= high1)],
                        color=color[1],
                        **intervals_kws) 
        plt.fill_between(x2d[np.logical_and(x2d >= low2, x2d <= high2)],
                        y2d[np.logical_and(x2d >= low2, x2d <= high2)],
                        color=color[2],
                        **intervals_kws) 
