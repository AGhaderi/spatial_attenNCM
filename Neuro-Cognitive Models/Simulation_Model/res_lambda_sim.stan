...
model {
    ...
    for (i in 1:N_obs + N_mis) { 
        ...
        y[i] ~ ratcliff_lpdf(alpha, res[cond_spat[i]] + lambda[cond_spat[i]]*n200lat[i], .5, delta[cond_coher[i]]);
    }
}
generated quantities { 
   ...
   // Wiener likelihood
    for (i in 1:N_obs+N_mis) {
        // Log density for DDM process
         log_lik[i] = ratcliff_lpdf(y[i] | alpha, res[cond_spat[i]] + lambda[cond_spat[i]]*n200lat[i], .5, delta[cond_coher[i]]) + n200lat_lpdf[i];
   }
}