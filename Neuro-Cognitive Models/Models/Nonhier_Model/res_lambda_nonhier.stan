...
model {
    ...
    for (c in 1:nspats){
        res[c] ~ normal(.1, .1) T[0,3];
        lambda[c] ~ normal(.4, .5) T[0,3];
    }

    for (c in 1:nconds) {
         n200sub[c] ~ normal(.17,.1) T[0,.4];
    }
    ...
    for (i in 1:N_obs + N_mis) { 
        ...
        y[i] ~ ratcliff_lpdf(alpha, res[cond_spat[i]] + lambda[cond_spat[i]]*n200lat[i], .5, delta[cond_coher[i]], eta);
    }
}
generated quantities { 
   ...
   // Wiener likelihood
    for (i in 1:N_obs+N_mis) {
        // Log density for DDM process
         log_lik[i] = ratcliff_lpdf(y[i] | alpha, res[cond_spat[i]] + lambda[cond_spat[i]]*n200lat[i], .5, delta[cond_coher[i]], eta) + n200lat_lpdf[i];
   }
}