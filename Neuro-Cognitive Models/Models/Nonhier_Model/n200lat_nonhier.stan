...
model {
    ...
    for (i in 1:N_obs + N_mis) { 
        ...
        y[i] ~ ratcliff_lpdf(alpha, res + lambda*n200lat[i], .5, delta[cond_coher[i]], eta);
    }
}
generated quantities { 
   ...
   // Wiener likelihood
    for (i in 1:N_obs+N_mis) {
        // Log density for DDM process
         log_lik[i] = ratcliff_lpdf(y[i] | alpha, res + lambda*n200lat[i], .5, delta[cond_coher[i]], eta) + n200lat_lpdf[i];
   }
}