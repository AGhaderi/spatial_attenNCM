...
model {
    ...
    for (i in 1:N_obs + N_mis) { 
        ...
        y[i] ~ ratcliff_lpdf(alpha[participant[i]], res[participant[i]] + lambda[participant[i]]*n200lat[i], .5, delta[participant[i],cond_coher[i]], eta[participant[i]]);
    }
}
generated quantities { 
   ...
   // Wiener likelihood
    for (i in 1:N_obs+N_mis) {
        // Log density for DDM process
         log_lik[i] = ratcliff_lpdf(y[i] | alpha[participant[i]], res[participant[i]] + lambda[participant[i]]*n200lat[i], .5, delta[participant[i],cond_coher[i]], eta[participant[i]]) + n200lat_lpdf[i];
    }
}