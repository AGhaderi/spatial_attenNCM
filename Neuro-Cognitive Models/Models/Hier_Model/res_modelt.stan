...
parameters {
    ...
    vector[ncohers] deltahier;  // Hierarchical drift rate for high and low coherences
    real alphahier;             // Hierarchical boundary
    real<lower=0> etahier;               // Hierarchical trial-to-trial standard deviation of drift rate
    vector[nspats] reshier;     // Hierarchical residual of Non-decision time
    vector<lower=0>[nconds] n200cond;    // Hierarchical between-condition for spatial attention in N200 latency
    real lambdahier;            // Hierarchical between-participant variability in coefficient of N200 latency 
    ...
    matrix[nparts,nspats] res;   // residual of Non-decision time for each participant and spatial attention
    matrix<lower=0, upper=.4>[nparts, nconds] n200sub;  // n200 mu parameter for each participant and spatial attention 
    vector[nparts] lambda;       // coefficient paramter for each participant
}
...
model {
    ...
    for (c in 1:nspats){
        reshier[c] ~ normal(.2,.4); 
    }
    
    lambdahier ~ normal(.5,2); 
    ...
    for (p in 1:nparts) {
        ...
        for (c in 1:nspats){
            res[p,c] ~ normal(reshier[c], ressd);
        }    
        lambda[p] ~ normal(lambdahier, lambdasd);
    ...
    for (i in 1:N_obs + N_mis) { 
        ...
        y[i] ~ ratcliff_lpdf(alpha[participant[i]], res[participant[i],cond_spat[i]] + lambda[participant[i]]*n200lat[i], .5, delta[participant[i],cond_coher[i]], eta[participant[i]]);
    }
}
generated quantities { 
   ...
   // Wiener likelihood
    for (i in 1:N_obs+N_mis) {
        // Log density for DDM process
         log_lik[i] = ratcliff_lpdf(y[i] | alpha[participant[i]], res[participant[i],cond_spat[i]] + lambda[participant[i]]*n200lat[i], .5, delta[participant[i],cond_coher[i]], eta[participant[i]]) + n200lat_lpdf[i];
    }
}