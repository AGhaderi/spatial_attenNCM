...
mis = np.where((data['n200lat']<.050)|(data['n200lat']>.250))[0] # missing data for n200lat
obs = np.where((data['n200lat']>.050)&(data['n200lat']<.250))[0] # observation and missing data for n200lat

N_mis = mis.shape[0]  # number of missing data
N_obs = obs.shape[0]   # number of observed data

modelfile = '../../stans/sim/n200lat_sim.stan' #reading the model span
f = open(modelfile, 'r')
model_wiener = f.read()
...
for c in range(0, nchains):
    chaininit = {
        'delta': np.random.uniform(.1, .2, size=ncohers),
...
utils.to_pickle(stan_model=sm, stan_fit=fit, save_path='../../save/sim/sim3_n200lat.pkl')
