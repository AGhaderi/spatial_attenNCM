...
modelfile = '../../stans/sim/res_lambda_all_sim.stan' #reading the model span
f = open(modelfile, 'r')
model_wiener = f.read()
...
for c in range(0, nchains):
    chaininit = {
        'delta_res': np.random.uniform(.1, .5, size=nconds),
        'delta_lambda': np.random.uniform(.1, .5, size=nconds),
        'alpha_res': np.random.uniform(.1, .5, size=nspats),
        'alpha_lambda': np.random.uniform(.1, .5, size=nspats),
...
utils.to_pickle(stan_model=sm, stan_fit=fit, save_path='../../save/sim/sim3_res_lambda_all.pkl')
