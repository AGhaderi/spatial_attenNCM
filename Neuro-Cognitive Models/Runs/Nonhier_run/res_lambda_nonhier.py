...
modelfile = '../../stans/res_lambda_nonhier.stan' #reading the model span
f = open(modelfile, 'r')
model_wiener = f.read()
...
for c in range(0, nchains):
    chaininit = {
        ...
        'lambda': np.random.uniform(.01, .02, size=nspats),
...
utils.to_pickle(stan_model=sm, stan_fit=fit, save_path='../../save/nonhier/'+str(parts)+'_res_lambda_nonhier.pkl')
