...
modelfile = '../../stans/res_lambda_nonhier_all.stan' #reading the model span
f = open(modelfile, 'r')
model_wiener = f.read()
...
for c in range(0, nchains):
    chaininit = {
        'delta_res': np.random.uniform(1, 3, size=nconds),
        'delta_lambda': np.random.uniform(1, 3, size=nconds),
        'alpha_res': np.random.uniform(.5, 1., size=nspats),
        'alpha_lambda': np.random.uniform(.5, 1., size=nspats),
        'eta_res': np.random.uniform(.01, .2, size=nspats),
        'eta_lambda': np.random.uniform(.5, .2, size=nspats),
        'tau_res': np.random.uniform(.01, .02, size=nspats),     
        'tau_lambda': np.random.uniform(.1, .2, size=nspats),     
...
utils.to_pickle(stan_model=sm, stan_fit=fit, save_path='../../save/nonhier/'+str(parts)+'_res_lambda_nonhier_all.pkl')
